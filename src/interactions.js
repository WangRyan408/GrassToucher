...
async function handleEdit(interaction, ctx) {
  ...
  if (title && thread.name !== name) await thread.setName(name);
  await rebuildDigest(ctx);
...
async function handleCancel(interaction, ctx) {
  ...
  const count = cancelRecipients(event, interaction.user.id).length;
  const notice = count
...
/**
 * DM everyone who might have turned up. Best effort on purpose: a member with server DMs
 * switched off answers 50007, and one closed inbox must not stop the rest.
 *
 * ponytail: one REST call per recipient. MAX_PER_LIST caps this near 80; batch only if it
 * ever costs real rate-limit headroom.
 */
async function notifyCancelled(ctx, event, actor, thread, recipients) {
  const when = `${time(event.startsAt, TimestampStyles.LongDateTime)} (${time(event.startsAt, TimestampStyles.RelativeTime)})`;
  const body = [
    `**Cancelled: ${escapeMarkdown(event.title)}**`,
    `In **${escapeMarkdown(ctx.guild.name)}** · was scheduled for ${when}`,
...
async function handleCancelConfirm(interaction, ctx) {
  ...
  const recipients = cancelRecipients(event, interaction.user.id);
  ...
  const { sent, failed } = await notifyCancelled(
    ctx,
    cancelled,
    interaction.user,
    thread,
  ...
  const notes = [`Cancelled **${escapeMarkdown(event.title)}**.`];
  if (sent) notes.push(`Notified ${sent}.`);
  if (failed) notes.push(`${failed} couldn't be DMed — their DMs are closed.`);
  return interaction.editReply({ content: notes.join(' '), components: [] });
}
